...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    EurekacoinTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= nCheckpointSpan)
        {
            ...
            eurekacoinDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = eurekacoinDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                eurekacoinDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                eurekacoinDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            eurekacoinDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            eurekacoinDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    EurekacoinDelegation eurekacoinDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= nCheckpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                eurekacoinDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = eurekacoinDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    eurekacoinDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    eurekacoinDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                eurekacoinDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(eurekacoinDelegations.GetDelegation(address, delegation) && EurekacoinDelegation::VerifyDelegation(address, delegation))
                    {
