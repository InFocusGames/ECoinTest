...
public:
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_eurekacoinDelegation.ExistDelegationContract() ? m_eurekacoinDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
                    validated = m_eurekacoinDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_eurekacoinDelegation.ExistDelegationContract() ? m_eurekacoinDelegation.GetDelegation(address, delegation) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_eurekacoinDelegation.VerifyDelegation(address, delegation);
    ...
    EurekacoinDelegation m_eurekacoinDelegation;
};
