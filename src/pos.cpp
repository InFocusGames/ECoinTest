...
EurekacoinDelegation& GetEurekacoinDelegation()
{
    static EurekacoinDelegation eurekacoinDelegation;
    return eurekacoinDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, BlockValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        EurekacoinDelegation& eurekacoinDelegation = GetEurekacoinDelegation();
        if(!eurekacoinDelegation.ExistDelegationContract())
        ...
        bool verifiedDelegation = eurekacoinDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee)
{
    Delegation delegation;
    EurekacoinDelegation& eurekacoinDelegation = GetEurekacoinDelegation();
    bool ret = eurekacoinDelegation.GetDelegation(address, delegation);
    if(ret) ret &= eurekacoinDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
