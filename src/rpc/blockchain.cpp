...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // eurekacoin
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // eurekacoin
    }
...
UniValue transactionReceiptToJSON(const EurekacoinTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if eurekacoind is started with -deprecatedrpc=size\n"
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        ...
        if (IsValidDestination(eurekacoinSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&eurekacoinSenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The eurekacoin address string"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
    ...
    if(!eurekacoinDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = eurekacoinDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The eurekacoin address string for staker"},
                },
               RPCResult{
            RPCResult::Type::ARR, "", "",
                {
    ...
    if(!eurekacoinDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = eurekacoinDelegation.DelegationsFromEvents(events);
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of eurekacoin addresses",
                                    {{RPCResult::Type::STR, "address", "eurekacoin address"}}},
                            }},
