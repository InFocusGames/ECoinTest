...
class EurekacoinDelegationPriv
{
public:
    EurekacoinDelegationPriv():
        m_pfDelegations(0),
    ...
    virtual ~EurekacoinDelegationPriv()
    {
        if(m_pfDelegations)
...
EurekacoinDelegation::EurekacoinDelegation():
    priv(0)
{
    priv = new EurekacoinDelegationPriv();
}
...
bool EurekacoinDelegation::GetDelegation(const uint160 &address, Delegation &delegation) const
{
...
bool EurekacoinDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool EurekacoinDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> EurekacoinDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void EurekacoinDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool EurekacoinDelegation::ExistDelegationContract() const
{
    // Delegation contract exist check
    return globalState && globalState->addressInUse(priv->delegationsAddress);
}

std::string EurekacoinDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool EurekacoinDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
