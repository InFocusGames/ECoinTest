...
ResultExecute EurekacoinState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, EurekacoinTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, EurekacoinTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, EurekacoinTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}
...
void EurekacoinState::validateTransfersWithChangeLog(){
...
void EurekacoinState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!EurekacoinState::addressInUse(delegationsAddress)){
        EurekacoinState::createContract(delegationsAddress);
        EurekacoinState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)});
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
